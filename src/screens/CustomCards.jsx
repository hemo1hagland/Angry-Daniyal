    ...
    <div className="flex min-h-full flex-col px-6 py-10">
        ...
        {MODES.map((m) => (
          <button
            ...
            className={`rounded-full px-4 py-2 font-body text-sm transition active:scale-95
              ${modus === m.id
                ? "bg-gradient-to-br from-ember to-gold text-ink font-semibold"
      ...
      <div className="mb-5 flex gap-2">
        <input
          ...
          maxLength={120}
          className="flex-1 rounded-2xl border border-white/10 bg-white/[0.04] px-5 py-4 font-body text-white placeholder-white/30 outline-none transition focus:border-ember/50"
        />
        <button
          onClick={leggTil}
      ...
      <div className="flex-1 space-y-2 overflow-y-auto">
        {liste.map((k, i) => (
          ...
          >
            ...
            <button
              onClick={() => fjern(i)}
              className="shrink-0 font-body text-sm text-white/30 transition hover:text-ember"
            >
              ✕
            </button>
          </div>
